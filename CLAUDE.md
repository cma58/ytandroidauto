...
| `data/SettingsRepository.kt` | DataStore wrapper — persists `sponsorblock_enabled` and `auto_sync_enabled`. Singleton object, use via `SettingsRepository.sponsorBlockEnabled(context)` etc. |
...
`SettingsRepository` wraps `androidx.datastore:datastore-preferences`. Keys: `sponsorblock_enabled`, `auto_sync_enabled`. `MainViewModel.connectToService()` starts collecting both flows into `_isSponsorBlockEnabled` and `_isAutoSyncEnabled` StateFlows on first call. `setSponsorBlock()` writes to DataStore + sends session command. `setAutoSync()` writes to DataStore + schedules/cancels a `PeriodicWorkRequest` (unique work name `"smart_sync"`, 6-hour interval, WiFi-only).

### searchResultsCache

`PlaybackService.searchResultsCache` is a `LinkedHashMap` capped at 20 entries using LRU eviction (`removeEldestEntry`). Prevents unbounded memory growth in long sessions.
...
```
datastore-preferences = 1.1.1  # SettingsRepository
